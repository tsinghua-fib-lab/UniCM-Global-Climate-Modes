...
def norm_std(data, norm_std):
    std = torch.ones([data[0].shape[-2], data[0].shape[-1]])
    data[0] = data[0]/std
    data[2] = data[2]/std
    return data, std
...
class make_embedding(nn.Module):
    ...
    def forward(self, x, timestamps):
        assert len(x.size()) == 4
        time_embed = self.time_emb(timestamps).unsqueeze(1)
        embedded_space = self.emb_space(self.spatial_pos)
        x = self.linear(x) + time_embed + embedded_space
        return self.norm(x)
...
class miniEncoder(nn.Module):
    def __init__(self, d_size, nheads, dim_feedforward, dropout, mode='ST'):
        super().__init__()
